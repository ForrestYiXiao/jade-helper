...
import com.zhipin.jadehelper.tool.CompareFileUtils;
import com.zhipin.jadehelper.tool.FileUtils;
import com.zhipin.jadehelper.tool.MessageDialogUtils;
...
//                return;
//            } else {
                String msg = String.format("文件 %s 已存在, 请选择处理方式?", file.getPath());
                int result = MessageDialogUtils.yesNoCancel(project, msg, "覆盖", "对比", "取消");
                switch (result) {
                    case Messages.YES:
                        // 覆盖文件
                        document = coverFile(file);
